...
import com.pixra.pixcore.support.PluginReloadUtil;
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (args.length > 0) {
            ...
            if (args[0].equalsIgnoreCase("reload")) {
                ...
                if (plugin.pearlCooldownManager != null) plugin.pearlCooldownManager.reload();
                if (plugin.tntMechanicsManager != null) plugin.tntMechanicsManager.reload();
                if (plugin.voidManager != null) plugin.voidManager.reload();
                if (plugin.pixFfaManager != null) {
                    plugin.pixFfaManager.reload();
                    plugin.pixFfaManager.bootstrapOnlinePlayers();
                }
            ...
            if (args[0].equalsIgnoreCase("arena")) {
                ...
                if (plugin.getMatchListener() == null) {
                    sender.sendMessage(ChatColor.RED + "[PixCore] MatchListener belum siap.");
                    return true;
                }

                if (args.length < 3) {
                    sender.sendMessage(ChatColor.RED + "Usage: /pix arena <snapshot|restore> <arena>");
                    return true;
                }
                ...
                if (action.equalsIgnoreCase("snapshot")) {
                    if (plugin.getMatchListener().saveArenaSnapshot(arenaName)) {
                        int blocks = plugin.getMatchListener().getArenaSnapshotBlockCount(arenaName);
                        sender.sendMessage(ChatColor.GREEN + "[PixCore] Snapshot arena '" + arenaName
                                + "' berhasil disimpan (" + blocks + " blocks).");
                        sender.sendMessage(ChatColor.GRAY + "Arena ini sekarang punya baseline bersih untuk restore otomatis.");
                    } else {
                        sender.sendMessage(ChatColor.RED + "[PixCore] Gagal menyimpan snapshot arena '" + arenaName + "'.");
                        sender.sendMessage(ChatColor.GRAY + "Pastikan nama arena valid dan area arenanya sudah rapi saat disimpan.");
                    }
                    return true;
                }

                if (action.equalsIgnoreCase("restore")) {
                    if (plugin.getMatchListener().restoreArenaSnapshot(arenaName)) {
                        sender.sendMessage(ChatColor.GREEN + "[PixCore] Arena '" + arenaName + "' berhasil direstore dari snapshot bersih.");
                    } else {
                        sender.sendMessage(ChatColor.RED + "[PixCore] Gagal restore arena '" + arenaName + "'.");
                        sender.sendMessage(ChatColor.GRAY + "Pastikan snapshot arena itu sudah pernah disimpan dengan /pix arena snapshot <arena>.");
                    }
                    return true;
                }
        ...
        sender.sendMessage(ChatColor.YELLOW + "/pix arena snapshot <arena>" + ChatColor.GRAY + " - Save clean arena baseline");
