...
public class HitActionBarManager implements Listener {
    ...
    public void onPlayerHit(EntityDamageByEntityEvent event) {
        ...
        try {
            ...
            if (plugin.pearlFightManager != null && plugin.pearlFightManager.isPearlFightKit(kitName)) {
                ...
                int currentLives = plugin.pearlFightManager.getLives(victim);
                int totalLives = plugin.pearlFightManager.getStartingLives();

                StringBuilder livesBar = new StringBuilder();
                if (currentLives > 0) {
                    livesBar.append(fullColor);
                    for (int i = 0; i < currentLives; i++) livesBar.append(heartSymbol);
                }
                if (totalLives - currentLives > 0) {
                    livesBar.append(emptyColor);
                    for (int i = 0; i < (totalLives - currentLives); i++) livesBar.append(heartSymbol);
                }

                String format = config.getString("message", "<player> &r<health_bar>");
                String nameColor = config.getString("player-name-color", "&e");
                String victimName = nameColor + plugin.getEffectivePlayerName(victim);
                String msg = format.replace("<player>", victimName)
                        .replace("<health_bar>", livesBar.toString())
                        .replace("<health>", String.valueOf(currentLives));

                sendActionBar(damager, ChatColor.translateAlternateColorCodes('&', msg));
                return;
            }
