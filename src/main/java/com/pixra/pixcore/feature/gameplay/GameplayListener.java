...
public class GameplayListener implements Listener {
    ...
    public void onBlockPlaceBuildLimit(BlockPlaceEvent event) {
        ...
        if (plugin.buildRestrictionsEnabled && plugin.arenaBoundaryManager != null) {
            if (plugin.isBuildLimitReached(player, event.getBlockPlaced())) {
                event.setCancelled(true);
                plugin.sendCooldownMessage(player, "build-limit-reached");
                return;
            }
            if (plugin.arenaBoundaryManager.checkArenaBorder(player, event.getBlockPlaced())) {
                event.setCancelled(true);
    ...
    public void onBlockPlaceBuildLimitPost(BlockPlaceEvent event) {
        Player player = event.getPlayer();

        if (!plugin.isHooked() || !plugin.buildRestrictionsEnabled || !plugin.isInFight(player)) return;
        if (plugin.frozenPlayers.contains(player.getUniqueId()) || plugin.activeCountdowns.containsKey(player.getUniqueId())) return;
        if (!plugin.isBuildFight(player) || plugin.arenaBoundaryManager == null) return;
        ...
        int strikePracticeBuildLimit = plugin.getStrikePracticeBuildLimit();
        if (plugin.arenaBoundaryManager.isPastStrikePracticeCenterLimit(player, event.getBlockPlaced(), strikePracticeBuildLimit)) {
